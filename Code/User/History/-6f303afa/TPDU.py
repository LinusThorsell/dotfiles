            ...
            if not product_id and item.get('rbservice_id'):
                ...
                if p.get('discount'):
                    if d.discount_percent:
                        i['amount'] += p['discount']
                    elif (i['amount'] + p['discount']) <= d.discount_fixed:
                        i['amount'] += p['discount']
                    elif i['amount'] <= d.discount_fixed:
                        i['amount'] = d.discount_fixed

        if i['amount']:
            dd.append(i)
            final_total -= i['amount']
...
def checkout(cart, member, method=None, total=None, is_anonym=False, option=None, items=None):
    ...
    with plug.db.commit_block(prevent_commit=True):
        ...
        fixed_discount = apply_discounts(payments, totals, member)
...
def apply_discounts(payments, totals, member):
    ...
    for dd in discount_details:
        ...
        if not discount.discount_percent and discount.discount_fixed: # Apply fixed amount discounts (like -X kr) once per transaction not per item.
            dp_count = get_dp(dd, payments, member)
            discountable_amount = get_discountable_amount(dd, payments, member)
            fixed_discount += min(discount.discount_fixed, discountable_amount)
        else: # % Based discounts work per item.
            for p in payments:
...
def get_dp(d, payments, m):
    for p in payments:
        prod_user = plug.user.get(p.get('user_id', m.id))
        if plug.discount.api.discountConditionsMet(p['cardtype_id'], d['prod_id'], prod_user):
            amount += p['amount']

def get_discountable_amount(discount, payments, member):
    amount = 0
    for p in payments:
        prod_user = plug.user.get(p.get('user_id', member.id))
        if plug.discount.api.discountConditionsMet(p['cardtype_id'], discount['prod_id'], prod_user):
            amount += p['amount']
    return amount

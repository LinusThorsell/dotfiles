...
class Booking { // Accessible as $booking or window.$booking
    ...
    getCards() {
        this.$api.get('/api/memberapi/trainingcard/get/all').then(cards => {
            ...
            cards.push({id: 192, someotherproperty: "sese2s3es"})

            console.log(cards[0].id)
            cards.forEach(element => {
                console.log(element.cardtype_name ? element.cardtype_name : element.id)
            });
            cards.sort((a,b) => {(a.cardtype_name ? a.cardtype_name : a.id).localeCompare((b.cardtype_name ? b.cardtype_name : b.id), "sv-SE")});

...
def apply_discounts(payments, totals, member):
    ...
    for dd in discount_details:
        ...
        if not discount.discount_percent and discount.discount_fixed: 
            discount_percent = get_percent_from_fixed(dd, payments, member)
            ...
            # discount.discount_fixed = None # <-- does some wacky things...
            # discount.discount_percent = float(discount_percent)*100.0
            fixed_discounts.append(discount)
        else:
            normal_discounts.append(discount)

    for discount in normal_discounts:
        for p in payments:
            prod_user = plug.user.get(p.get('user_id', member.id))
            if plug.discount.api.discountConditionsMet(p['cardtype_id'], dd['prod_id'], prod_user):
...
def get_percent_from_fixed(discount_details, payments, member):
    # Calculate the Total applicable payments
    total_payment = 0
    for p in payments:
        prod_user = plug.user.get(p.get('user_id', member.id))
        if plug.discount.api.discountConditionsMet(p['cardtype_id'], discount_details['prod_id'], prod_user):
            total_payment += p['amount']
    return discount_details['amount'] / total_payment

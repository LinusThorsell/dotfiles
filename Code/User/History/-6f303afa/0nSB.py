...
def apply_discounts(payments, totals, member):
    ...
    for dd in discount_details:
        #max_discount = dd['amount']
        valid_payments = []
        discount = db.session.query(db.TrainingCardType).get(dd['prod_id'])
        prod_user = plug.user.get(payments[0].get('user_id', member.id))
        ...
        plug.discount.api.apply_discount(valid_payments, discount, prod_user, deduct_discount=False, from_date=from_date)
